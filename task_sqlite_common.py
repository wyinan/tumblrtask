...
def _concat_keys(keys):
    _args = keys
    _key_list = _args
    while not isinstance(_args, str):
        _key_list = _args
        _args = _args[0]

    _key_list = list(map(lambda _key: "[" + _key + "]" if _key.find('(') == -1 else _key, _key_list))
...
class DataHelper(object):
    ...
    def where(self, conditions, _operate="AND"):
        if not conditions:
            return self
        elif isinstance(conditions, list):
            conditions = list(map(DataCondition.String, conditions))
            self.condition_snippet = (" %s " % _operate).join(conditions)
        else:
            self.condition_snippet = conditions
        self.__where()
